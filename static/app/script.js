...
var x1px = 1;
var y1px = 1;
var worker = new Worker("/static/app/worker.js");
...
function updateDataNodes() {
  ...
  var spriteData = (map.style && map.style.sprite && map.style.sprite.data) ? map.style.sprite.data : {};
  var nodesFeatures = [];
...
d3.json(base_url+"/api/v1/category/?format=json&limit=5000").get(function (error, data) {
  if (error) {
...
d3.json(base_url+"/api/v1/item/?format=json&limit=5000").get(function (error, data) {
  if (error) {
...
d3.json(base_url+"/api/v1/atlas/?format=json&limit=5000").get(function (error, data) {
  if (error) {
    console.error("Error loading atlas:", error);
    return;
  }
  var s = _.first(_.filter(data.objects, function _filter(obj,index){
    return obj.pixel_ratio == pixelRatio && obj.sprite_width == spriteWidth;
  }));
  if (!s) {
    console.warn("No atlas found for pixelRatio=" + pixelRatio + " spriteWidth=" + spriteWidth + ", using first available");
    s = data.objects[0];
  }
  if (s && s.atlas_map_url) {
    sprite = base_url + s.atlas_map_url.replace('.json','');
  } else {
    console.error("Invalid atlas data:", s);
    sprite = base_url + "/static/app/atlas_30_1";
  }
  boot();
})
var u = normalizeStyleURL(mapboxglStyle);

d3.json(u).get(function(error, data) {
  if (error) {
...
function showItem() {
  ...
  _.range(1, 10).map(function _map(k, i2) {
    ...
    // Use original media files from cms_archive instead of missing thumbnails
    var url = base_url+'/media/cms_archive/' + filename;
